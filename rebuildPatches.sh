...
process_changes "./baguette-server/src/minecraft/java" "Minecraft"

gradle_rebuild_task=false

if $force_run || ! git diff --quiet "./baguette-server/build.gradle.kts" || ! git diff --cached --quiet "./baguette-server/build.gradle.kts"; then
  echo "Changes detected in ./baguette-server/build.gradle.kts"
  gradle_rebuild_task=true
fi

if $force_run || ! git diff --quiet "./baguette-api/build.gradle.kts" || ! git diff --cached --quiet "./baguette-api/build.gradle.kts"; then
  echo "Changes detected in ./baguette-api/build.gradle.kts"
  gradle_rebuild_task=true
fi
